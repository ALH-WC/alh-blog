...
// ---------- static seams image ----------
// The street network, canals, A10 and parks go into a static SVG file served
// from /public rather than into the JS module. The browser rasterizes an <img>
// once and composites the cached bitmap, so hovering the interactive blocks
// repaints 26 simple polygons instead of re-walking two hundred thousand street
// points, and the JS bundle stops carrying the network entirely.
const SEAM = PAPER;
const seamsSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}">
...
fs.writeFileSync(path.join(PUB, 'map-seams.svg'), seamsSvg);
...
console.log(`wrote public/map-seams.svg (${(seamsSvg.length / 1024).toFixed(1)}kb, cached raster)`);
