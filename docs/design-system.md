...
- **4.14 Employer logo rows**: REAL client-supplied logos (`/public/logos/`, 18 companies, source files in `Website/Logos/`). Rendered as images at uniform 26px height (24px in static bars, 20px phones) with `filter: brightness(0); opacity: 0.55`: every logo becomes one quiet near-black tint, never brand colors. The homepage marquee ("Our clients work at companies like") rotates the full set; static bars (b2b "Trusted by teams at") show a subset by name from `src/lib/logos.ts`. Bloomreach and TomTom are out (no logo delivered).
