...
9. **Stats appear ONLY in the hero pile** and are exactly: 250+ expats housed, 3.5 wks average search, 8+ yrs of experience, 85% from referrals. No stats bands in bodies; heroless pages simply have no stats.
...
- **/ (home)**: hero video (+ pile, Cal.com button) → about split "Foreigners ourselves." (team photo right) → services (uppercase head + cards, third on sand, plain-line lists) → dark corporate strip (the page's one dark band) → three review cells + "Read all our reviews →" → employers row → closing invitation (uppercase Mixta 62-44, text CTA, muted note) → guide band → footer. NO form, NO stats band.
- **/renting**: the fullest page: gate (plain lines) → abroad/here → process rows (white panel) → this-is-us (Mixta 400 32px quote) → endless review carousel (quiet cells) → fee split (white, hairlines) → tiles → guide band → FAQ 2-col → contact form. Keeps its carousel signature.
