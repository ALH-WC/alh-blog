...
- Nav hide/show `translateY(-100%)` 0.38s; pop-up entrance opacity + 18px rise, 0.55s; marquee ~26s linear, NEVER pauses (hover-pause removed, feedback Aug 2026).
...
- **4.7 Service tile**: min-height 320px, padding 40px, bronze numeral top-left, Mixta 34px name + one-line promise pinned to the bottom, arrow right-aligned on the promise line. ALL tiles white; sand #EAE2D3 is the HOVER state (no lift: filled cells never move). Sand hover marks CLICKABLE white blocks only (tiles, the guide tile); non-clickable blocks (review cells, FAQ rows) never tint. No standing sand tile. Tile rows bleed full width with hairlines.
...
- **4.13 Guide band**: the one guide gateway. Service pages carry it as the last band before the footer (white, top/bottom hairlines, the signature reveal). The HOMEPAGE carries it as a CELL inside the photo band instead (third position, hairlines left AND bottom, bottom-aligned content, h3 at 34px): never buried at the page bottom there, and never twice on one page. In the tile the category block FADES UP once when the tile is FULLY in view (IntersectionObserver at 90%, opacity + 14px rise, 0.7s + 0.1s delay): the signature clip reveal is NOT used there (it fought the wrapped layout). Nothing in the tile is bold: title, label, and categories all weight 400. The homepage pop-up is SCROLL-LINKED to the photo band (popAnchor on ServiceShell, anchor id pop-anchor): while sliding in, its top edge stays a constant 28px BELOW the band's bottom hairline (1px scroll = 1px movement, the blog-handoff principle; opacity is the only self-animated property), so it never overlaps the band; once fully in it stays fixed and rides along. The tile carries no Learn-about eyebrow (the band on service pages keeps it). Other pages keep the 25% scroll threshold.
- **4.13b The pop-up (Aug 2026, the bar form)**: a horizontal espresso bar bottom-right, NOT dismissible (no ×): the centred muted line "Let's talk! We respond within 24 hours." on top, then two buttons side by side, cream filled "Fill in our form" (per-page formHref) + outlined "Schedule a free video intake call". It appears right after the hero (anchored pages) or at the popAt depth, and hides while the footer is in view so the newsletter stays reachable. On phones it spans edge to edge and the buttons stack. Squared corners, our colors: the reference's red pill bar translated into the Quiet System.
- **4.13c The homepage photo band** is now [neighborhoods-article photo cell] [BSN-article photo cell] [guide tile]: the left cell is the canal-houses photo captioned "Top neighborhoods in Amsterdam for expats to live" (21px caption variant) linking to that article; the middle cell is the leafy-lane photo (calm ground so the caption reads) linking to the BSN article.
