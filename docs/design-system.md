...
- **4.13 Guide band**: the one guide gateway. Service pages carry it as the last band before the footer (white, top/bottom hairlines, the signature reveal). The HOMEPAGE carries it as a CELL inside the photo band instead (third position, hairline-left, bottom-aligned content, h3 at 34px, reveal capped at 520px with the category list allowed to wrap at 13px): never buried at the page bottom there, and never twice on one page.
