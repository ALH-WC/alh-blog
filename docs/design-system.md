...
- **4.13b The pop-up's life cycle (Aug 2026)**: right after the hero it appears as a LOW bar showing only the title ("Let's talk."); hovering expands it temporarily; once the page's guide block has scrolled past it expands for good; while the footer is in view it collapses back to the bar so the newsletter stays reachable (this is the answer to the footer-overlap problem). Implemented as `.ctapopMini` + `.ctapopBody` in ServiceShell; /renting's own pop-up hides near the footer.
