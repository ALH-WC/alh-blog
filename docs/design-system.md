...
- **4.14 Employer logo rows**: REAL client-supplied logos (`/public/logos/`, 18 companies, source files in `Website/Logos/`). Rendered as images with a per-logo OPTICAL height (24-36px, in `src/lib/logos.ts`: wordmark-only logos get more height than icon-plus-text logos so all look the same size; phones scale to 80%) and `filter: grayscale(1) brightness(0.52); opacity: 0.72`: one quiet dark tint that PRESERVES white knockout details inside dark marks (a flat brightness(0) erased Miro's M and Reddit's face). Never brand colors. The homepage marquee ("Our clients work at companies like") rotates the full set; static bars (b2b "Trusted by teams at") show a subset by name from `src/lib/logos.ts`. Bloomreach and TomTom are out (no logo delivered).
