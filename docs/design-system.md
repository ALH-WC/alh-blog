...
- **4.13 Guide band**: white with top/bottom hairlines, Mixta title + bronze ↗, the signature reveal kept. No sand fill. It is the LAST band before the footer on EVERY company page, the homepage and /renting included: one identical guide gateway sitewide. The homepage carries NO separate guide module (the three-article version was removed as a duplicate); the band is the single guide gateway everywhere.
- **4.14 Employer logo rows**: REAL client-supplied logos (`/public/logos/`, 18 companies, source files in `Website/Logos/`). Rendered as images with a per-logo OPTICAL height (24-36px, in `src/lib/logos.ts`: wordmark-only logos get more height than icon-plus-text logos, and files with internal padding get compensated heights measured from their content bounding box (Google 54, Deliverect 60); phones scale to 80%) and `filter: grayscale(1) brightness(0.28); opacity: 0.78` (measured so every logo, light originals included, lands under 0.2 luminance while knockout details stay visible): one quiet dark tint that PRESERVES white knockout details inside dark marks (a flat brightness(0) erased Miro's M and Reddit's face). Never brand colors. The homepage marquee ("Our clients work at companies like") rotates the full set; static bars (b2b "Trusted by teams at") show a subset by name from `src/lib/logos.ts`. Bloomreach and TomTom are out (no logo delivered). Logo files are PREPARED, not just styled: every SVG viewBox is trimmed to the artwork (no baked-in padding), marks with large filled containers (Miro square, Reddit Snoo, ABN shield) use their wordmark-only lockup, and each logo carries a per-logo optical height in src/lib/logos.ts calibrated to the Yandex wordmark at 28px (caps-dense marks 22-24, bold wordmarks 26, thin serif and stacked marks 30). Marquee gap is a constant 76px.
