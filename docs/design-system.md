...
- **4.13b The pop-up (Aug 2026, the bar form)**: a horizontal espresso bar bottom-right (no title): two buttons side by side, cream filled "Fill in our form" (per-page formHref) + outlined "Schedule a free video intake call", with the centred muted note "We respond within 4 hours." underneath and the dismiss ×. It appears right after the hero (anchored pages) or at the popAt depth, and hides while the footer is in view so the newsletter stays reachable. On phones it spans edge to edge and the buttons stack. Squared corners, our colors: the reference's red pill bar translated into the Quiet System.
