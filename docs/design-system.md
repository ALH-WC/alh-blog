...
- **4.13b The pop-up (Aug 2026, the bar form)**: a horizontal espresso bar bottom-right, NOT dismissible (no ×): the centred muted line "Let's talk! We respond within 4 hours." on top, then two buttons side by side, cream filled "Fill in our form" (per-page formHref) + outlined "Schedule a free video intake call". It appears right after the hero (anchored pages) or at the popAt depth, and hides while the footer is in view so the newsletter stays reachable. On phones it spans edge to edge and the buttons stack. Squared corners, our colors: the reference's red pill bar translated into the Quiet System.
- **4.13c The homepage photo band** is now [neighborhoods-map cell] [BSN-article photo cell] [guide tile]: the map cell is white (sand on hover) with the illustrated map contained and an ink caption "Amsterdam neighborhoods", linking to /blog#sec-citymap; the middle cell keeps the bicycles photo captioned with the article title (21px variant), linking to the BSN article.
